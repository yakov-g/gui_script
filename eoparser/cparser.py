  ...
  def h_file_data_get(self, filename):
   ...
   #fetch all "#typedef enum" from file
    ...
    for tup in ss:
      ...
      s_tmp = tup[0]
      s_tmp = s_tmp.replace("\n", "").replace("\\", "")
      s_tmp = re.sub("/\*.*?\*/", "", s_tmp) #remove comments
      s_tmp = " ".join(s_tmp.split())
      td_name = s_tmp[s_tmp.find("}") + 1 : s_tmp.find(";")].strip()
      td_values = s_tmp[s_tmp.find("{") + 1 : s_tmp.find("}")].strip().split(",")
      td_values = filter(len, td_values)
      td_list = []
      for v in td_values:
         v = v.strip()
         match = re.search("[\w]*", v);
         td_list.append(match.group(0))

      self.typedef_enum[td_name] = (filename, td_list)
